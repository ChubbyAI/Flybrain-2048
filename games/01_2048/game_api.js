(function () {
  ...
  function prepareEpisode(options) {
    ...
    const notes = [];
    const previousSeed = getCurrentSeed();
    ...
    return {
      accepted: accepted,
      appliedSeed: appliedSeed === null ? getCurrentSeed() : appliedSeed,
      previousSeed: previousSeed,
      notes: notes
    };
  }

  function commitPreparedEpisode(prepared) {
    session.seed = prepared.appliedSeed;
    session.requestedLevel = prepared.accepted.level;
    session.requestedDifficulty = prepared.accepted.difficulty;
    session.episodeStartMs = Date.now();
    session.episodeCount += 1;
    return {
      accepted: prepared.accepted,
      applied: { seed: session.seed, level: null, difficulty: null },
      notes: prepared.notes
    };
  }

  function rollbackPreparedEpisode(prepared) {
    applySeed(prepared.previousSeed);
  }

  function resetPostcondition(manager, previousGrid) {
    if (!manager || !manager.grid || manager.grid === previousGrid) return false;
    const board = serializeBoard(manager);
    return (
      board.length === manager.size &&
      countFilled(board) === 2 &&
      finiteOrNull(manager.score) === 0 &&
      manager.over === false &&
      manager.won === false
    );
  }
  ...
  window.gameAPI = {
    ...
    init: async function init(config) {
      const prepared = prepareEpisode(config);
      const manager = await waitForManager(1000);
      const previousGrid = manager ? manager.grid : null;
      const started = !!manager && restartGame();
      const verified = started && resetPostcondition(manager, previousGrid);
      const episode = verified ? commitPreparedEpisode(prepared) : null;
      if (!verified) rollbackPreparedEpisode(prepared);
      runtime.lastResetMethod = verified ? 'inplace' : 'unsupported';
      return {
        ok: verified,
        accepted: prepared.accepted,
        applied: episode
          ? episode.applied
          : { seed: session.seed, level: null, difficulty: null },
        notes: verified
          ? (episode.notes.length ? episode.notes : [])
          : prepared.notes.concat(['native_reset_postcondition_failed'])
      };
    },
    ...
    reset: async function reset(options) {
      ...
      return {
        ok: verified,
        method: verified ? 'inplace' : 'unsupported',
        accepted: prepared.accepted,
        applied: episode
          ? episode.applied
          : { seed: session.seed, level: null, difficulty: null },
    ...
    restart: function restart() {
      const prepared = prepareEpisode(null);
      const manager = getManager();
      const previousGrid = manager ? manager.grid : null;
